...
const createPointCloud = (
  ...
  const nodeIds = new Uint16Array(nodes.length);
...
const createCoreConnections = (nodes: NeuralCoreNode[]): NeuralCoreConnection[] => {
  const highlights = nodes
    .filter((node) => (
      node.id !== CORE_NODE_ID
      && (node.kind === "hub" || node.kind === "core")
    ))
    .slice(0, 18);
...
const getConnectionPairKey = (connection: NeuralCoreConnection): string => {
  const firstNodeId = Math.min(connection.fromNodeId, connection.toNodeId);
  const secondNodeId = Math.max(connection.fromNodeId, connection.toNodeId);
  return `${firstNodeId}:${secondNodeId}`;
};

const shouldReplaceConnection = (
  current: NeuralCoreConnection,
  candidate: NeuralCoreConnection,
): boolean => {
  if (candidate.strength !== current.strength) {
    return candidate.strength > current.strength;
  }

  if (candidate.opacity !== current.opacity) {
    return candidate.opacity > current.opacity;
  }
...
const deduplicateConnections = (
  connections: readonly NeuralCoreConnection[],
): NeuralCoreConnection[] => {
  const connectionByNodePair = new Map<string, NeuralCoreConnection>();
  ...
  for (const connection of connections) {
    ...
    const current = connectionByNodePair.get(pairKey);
    if (!current || shouldReplaceConnection(current, connection)) {
      connectionByNodePair.set(pairKey, connection);
    }
  }

  return [...connectionByNodePair.values()];
};
...
const createConnectionBuffers = (connections: NeuralCoreConnection[]): NeuralCoreConnectionBuffer[] => {
  ...
  return buckets
    .map((bucket) => {
      ...
      const fromNodeIds = new Uint16Array(bucketConnections.length);
      const toNodeIds = new Uint16Array(bucketConnections.length);
      ...
      bucketConnections.forEach((connection, index): void => {
        ...
        fromNodeIds[index] = connection.fromNodeId;
        toNodeIds[index] = connection.toNodeId;
      });

      return {
        bucket,
        positions,
        connectionIds: bucketConnections.map(({ id }) => id),
        fromNodeIds,
        toNodeIds,
